    ...
    <div className={`rounded-2xl border px-5 py-4 ${req.satisfied ? 'bg-green-50 border-green-100' : 'bg-white border-gray-100 shadow-sm'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {req.no_articulation && (
            <p className="text-xs text-gray-400 italic">No articulation available</p>
          )}
      ...
      >
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Not required for admission, but completing these saves time - {courses.length} courses, {done} already done
          </p>
        </div>
  ...
  return (
    <div className="mb-10">
      <div className="flex items-center justify-between mb-2">
        <div>
          <h3 className="text-lg font-bold text-gray-900">{result.school_name}</h3>
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm">Based on your classes and selected schools.</p>
        </div>
        <button onClick={load} className="text-sm text-indigo-600 hover:text-indigo-800 font-medium transition-colors">
          Refresh
        </button>
      </div>
              ...
              {totalRemaining > 0 && (
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-5 py-4 mb-8 flex items-center gap-4">
                  ...
                  <p className="text-xs text-gray-500 leading-relaxed">
                    Each item shows what De Anza or Foothill class satisfies it. If multiple options appear, you only need one.
                  </p>
                </div>
              )}
              {results.map((r, i) => (
