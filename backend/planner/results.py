...
def _parse_advisory(template: list, valid_ucsd_codes: set, agreement_key: str = '', receiving_id: int = None, major_name: str = ''):
    ...
    if receiving_id == 76 and major_name:
        try:
            from .csula_scraper import fetch_csula_requirements
            result = fetch_csula_requirements(major_name, valid_ucsd_codes)
            if result and (result['required'] or result['recommended'] or result['choose_one_groups'] or result['series_groups'] or result.get('flags')):
                return result
        except Exception:
            pass
    ...
    if receiving_id == 81 and major_name:
        try:
            from .csulb_scraper import fetch_csulb_requirements
            result = fetch_csulb_requirements(major_name, valid_ucsd_codes)
            if result and (result['required'] or result['recommended'] or result['choose_one_groups'] or result['series_groups'] or result.get('flags')):
                return result
        except Exception:
            pass

    if receiving_id == 26 and major_name:
        try:
            from .sdsu_scraper import fetch_sdsu_requirements
            result = fetch_sdsu_requirements(major_name, valid_ucsd_codes)
            if result and (result['required'] or result['recommended'] or result['choose_one_groups'] or result['series_groups'] or result.get('flags')):
                return result
        except Exception:
            pass
